...
class Params:
    ...
    resume = '/home/macaodha/Projects/inat2018/code/github_code/inat_comp_2018-master/iNat_2018_InceptionV3.pth.tar'
    ...
    save_preds = True
    split_name = 'val'
    op_file_name = 'net_feats/inat2018_'+split_name+'_net_feats' 
    if evaluate == True:
        val_file = '/media/Storage_1/Datasets/inat2018/'+split_name+'2018.json'
...
def main():
    ...
    if args.evaluate:
        prec3, preds, im_ids, feats = validate(val_loader, model, criterion, True)
        # write predictions to file
        if args.save_preds:
            np.save(args.op_file_name[:-4], feats)
...
def validate(val_loader, model, criterion, save_preds=False):
    ...
    if save_preds:
        return top3.avg, np.vstack(pred), np.hstack(im_ids), np.vstack(feats)
    else:
        return top3.avg
